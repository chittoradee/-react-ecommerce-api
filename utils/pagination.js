...
const paginate = async (page,limit,name,model,api='') => {
    ...
    {   
        ...
        const allDocs = await model.find({ "name": { $regex: ".*" + name + ".*" , '$options' : 'i' }});
        totalRecords = allDocs.length;
         
        // paginated result
        result.data = await model.find({ "name": { $regex: ".*" + name + ".*" , '$options' : 'i' }}).collation({locale: "en" }).select(["image","name"]).limit(limit).skip(startIndex).sort({name : 1}).
        exec();
    ...
    // pagination for getAllItemsForPin api
    else if( api === 'getAllItemsForPin' )
    {
